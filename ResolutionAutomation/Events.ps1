...
function OnStreamStart() {
    ...
    # Fall back to parsing Sunshine log if env vars are not available (persistent daemon re-detection)
    if (-not $width -or -not $height) {
        $logRes = Get-LastClientResolution
        if ($logRes) {
            Write-Host "Parsed client resolution from Sunshine log: $($logRes.Width)x$($logRes.Height)x$($logRes.Refresh)"
            $width = $logRes.Width
            $height = $logRes.Height
            $refresh = $logRes.Refresh
        }
    }
...
function Assert-ResolutionChange($width, $height, $refresh) {
    ...
    $maxAttempts = $settings.resolutionCheckAttempts
    $sleepMs = $settings.resolutionCheckIntervalMs
    Write-Debug "Resolution check: $maxAttempts attempts, $sleepMs ms interval"
